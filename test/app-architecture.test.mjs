...
test("Next.js owns the approved V6.39 mobile client and V7.3 desktop workspace", () => {
  ...
  assert.ok(navigation.indexOf('label: "Agent 组件"') < navigation.indexOf('href: "\/app\/workers\/calendar"'));
  ...
  assert.match(read("core/app/src/components/desktop-v627/plans-page.tsx"), /<CalendarPage/);
  ...
  assert.doesNotMatch(taskViewNavigation, /\/plans/);
  assert.match(taskViewNavigation, /\/calendar/);
  assert.match(taskViewNavigation, /执行记录/);
  assert.match(workersClient, /CalendarModuleHeader active="tasks"/);
  assert.match(workersClient, /calendar-module-page[\s\S]*CalendarModuleHeader/);
  assert.match(scheduledTasksClient, /<PlansPage/);
  assert.doesNotMatch(read("core/app/src/components/desktop-v627/plans-page.tsx"), /PlanList|active="plans"/);
