...
test('validates duplex invariants and rejects broken references or geometry', () => {
  ...
  const intrudingSlab = structuredClone(fixture); intrudingSlab.slabs[0].polygon = [[3,0],[7,0],[7,6],[3,6]];
  assert.match(validateModel(intrudingSlab).join('\n'), /upper slab overlaps void living-void/);
  const secondTallRoom = structuredClone(fixture); secondTallRoom.rooms.find((room) => room.id === 'bedroom').height = 3.4;
  assert.match(validateModel(secondTallRoom).join('\n'), /non-target lower height must be below 3.2/);
  const wrongVoid = structuredClone(fixture); wrongVoid.voids[0].roomId = 'bedroom';
  assert.match(validateModel(wrongVoid).join('\n'), /roomId must equal living/);
});
...
test('normalizes duplex coordinates, vertical geometry and per-level area', () => {
  ...
  assert.equal(normalized.assertions.doubleHeightM, 12);
...
test('keeps legacy single-level models backward compatible', () => {
  ...
  delete legacy.levels; delete legacy.slabs; delete legacy.voids; delete legacy.stairs; delete legacy.railings; delete legacy.presentation; delete legacy.assertions;
...
test('generates a self-contained accessible orientation-aware viewer', () => {
  ...
  assert.match(html, /旋转手机以横屏查看完整户型/);
  assert.match(html, /id="continue-portrait"/);
  assert.match(html, /orientationchange/);
  assert.match(html, /orientation\.lock/);
