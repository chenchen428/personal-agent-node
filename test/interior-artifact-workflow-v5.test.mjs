...
test("V5 creates six editable drawings and one per-view panorama chain", () => {
  ...
  assert.deepEqual(state.artifacts["panorama-photorealistic-living"].dependsOn, ["panorama-imagegen-prompt-living"]);
  ...
  assert.deepEqual(state.artifacts["panorama-hotspots-living"].dependsOn, ["panorama-camera-living", "panorama-photorealistic-living"]);
...
test("workflow summary reports unfinished artifacts", () => {
  ...
  assert.equal(summary.counts.draft, 20);
