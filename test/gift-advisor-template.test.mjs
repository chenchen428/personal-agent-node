...
import { validatePageTemplateArtifact } from "../core/agent/src/online-pages/template-catalog.js";
import {
  buildGiftAdvisorTemplateExample,
} from "../skills/gift-advisor/scripts/build-template-example.mjs";
import {
...
const artifactRoot = path.join(root, "core/app/public/assets/templates/gift-advisor-report-v1");
const examplePlanPath = path.join(root, "skills/gift-advisor/examples/template-plan.json");

test("the committed gift advisor example is a byte-stable governed template artifact", () => {
...
test("gift generator confines output to the project derived directory and produces Page provenance", () => {
  ...
  try {
    ...
    const result = generateGiftAdvisorPage({ projectDir, output, template: "gift-advisor-report" });
    assert.equal(result.ok, true);
    assert.equal(result.template.visualAcceptance, "user");
    const html = fs.readFileSync(path.join(output, "index.html"));
    const verified = validatePageTemplateArtifact("gift-advisor-report", html);
    assert.equal(verified.provenance.id, "gift-advisor-report");
    assert.equal(verified.provenance.version, 1);
    assert.equal(verified.provenance.artifactSha256, result.artifactSha256);
    assert.throws(
      () => generateGiftAdvisorPage({
        projectDir,
