...
import { detectRequestSurface } from "../core/app/src/lib/request-surface.ts";
...
test("request surface prefers client hints and classifies common mobile devices before render", () => {
  assert.equal(detectRequestSurface(headers({ "sec-ch-ua-mobile": "?1", "user-agent": "Windows NT" })), "mobile");
  assert.equal(detectRequestSurface(headers({ "sec-ch-ua-mobile": "?0", "user-agent": "iPhone" })), "desktop");
  assert.equal(detectRequestSurface(headers({ "user-agent": "Mozilla/5.0 (Linux; Android 14; Pixel 8) Mobile" })), "mobile");
  assert.equal(detectRequestSurface(headers({ "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" })), "desktop");
  assert.equal(detectRequestSurface(headers({})), "unknown");
});
...
test("mobile routes use direct modules and a persistent parent shell", () => {
  ...
  for (const file of fs.readdirSync(path.join(root, "core/app/src/app/app/mobile"), { recursive: true }).filter((name) => String(name).endsWith("page.tsx"))) {
    ...
    assert.doesNotMatch(source, /from "@\/components\/mobile-current"/);
  }
});
