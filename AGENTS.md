...
Memory is a separate, Space-local product domain for durable facts, stable preferences, and continuing constraints. It is not Activity, conversation history, a tool log, or a vector side channel.

- Every Space owns a separate memory store, indexes, hit ledger, heat calculation, forgetting state, and read-only UI projection. Cross-Space search, recall, mutation, migration, or shared indexes fail closed even on loopback.
- The business record stores only memory content. System metadata may include stable ID, Space ID, verified main session, status, hit count, heat, last hit, forgetting time, revision, creation time, and update time. Do not add title, type, tags, source, applicable scope, or user-editable policy fields.
- Status is `active` or `forgotten`. Active records are ordered by heat; forgotten records are ordered by forgetting time. The authenticated UI may list, search, and inspect both states but never create, edit, reactivate, forget, or delete.
