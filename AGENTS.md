...
- Remove the legacy `memories` table and store methods, `/agent-memory`, `/agent-bridge/memory`, `/api/memory-sessions`, `/api/memories*`, `open-abg memory ...`, memory-management UI, orchestration instructions, and memory-specific tests from the target product.
...
The installed `personal-agent` CLI is the stable Agent automation contract. Read `skills/personal-agent/SKILL.md`, start with `personal-agent status --json` and capability discovery, and use only registered, executable commands. Do not call internal HTTP ports, inspect internal databases, or fall back to legacy `open-abg` or `private-site` commands when an Activity command is unavailable. Report the capability gap honestly and preserve the underlying task result; never fabricate a successful activity write.
...
- When attached to the private Personal Agent workspace, follow its root `DESIGN.md` and update `docs/prototypes/desktop/` or `docs/prototypes/mobile/`, with one independently openable HTML per primary page. A standalone Node clone must remain self-contained; if the parent design workspace is unavailable, create a temporary review artifact under its own `docs/design/` and obtain explicit approval before implementation.
