...
`/` or to an installer-provided loopback URL. The gateway resolves `/` to a compatible default
Personal App or the official `/app` recovery Console. Only that exact loopback origin stays inside
the window. Other HTTP(S) links open in the system browser, and no Tauri command capability is exposed
to Web content.
