...
const featuredRoutes: Record<string, { preview: string; device: "desktop" | "mobile" }> = {
  ...
  "travel-planner": { preview: "/assets/agents/travel-planner/featured/index.html", device: "mobile" },
};
  ...
  return <div className="agent-featured-output">
    ...
    <section className={`agent-example-stage ${mobile ? "is-mobile" : "is-desktop"}`} aria-label={selected.title}>
      ...
      <div className="agent-example-canvas">
        {preview ? <iframe title={selected.title} src={preview} sandbox="allow-scripts allow-same-origin" /> : <div className="agent-example-placeholder"><FileText aria-hidden="true" /><strong>{selected.title}</strong><p>{selected.description || selected.summary}</p></div>}
      </div>
    </section>
  </div>;
}
