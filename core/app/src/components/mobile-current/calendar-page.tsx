...
export function MobileCalendarPage() {
  ...
  return <MobileListShell section="calendar" title="日程" note="你的安排，Cove 持续跟进" query={calendar.query} setQuery={calendar.setQuery} searchLabel="搜索日程" searchPlaceholder="搜索安排、参与人" filter={{ label: "日程状态", description: "选择要查看的安排", value: calendar.status, setValue: calendar.setStatus, options: [{ value: "all", label: calendar.period === "upcoming" ? "全部未结束安排" : "全部" }, ...calendar.statusOptions.map(([value, label]) => ({ value, label }))] }}>
    <div className="mobile-calendar">
      ...
      <div className="mobile-calendar-toolbar">{calendar.period !== "upcoming" ? <><button aria-label="上一个时间段" onClick={() => calendar.move(-1)}><ChevronLeft size={18} /></button><input type="date" aria-label="日程日期" value={calendar.day} onChange={(event) => calendar.setDay(event.target.value)} /><button aria-label="下一个时间段" onClick={() => calendar.move(1)}><ChevronRight size={18} /></button></> : <span>从现在起 · 按时间排序</span>}<button aria-label="刷新日程" onClick={() => void calendar.refresh()}><RefreshCw size={17} /></button></div>
      <div className="mobile-calendar-caption"><button onClick={calendar.today}>今天</button><span>{calendar.value?.total ?? "—"} 项 · {calendar.timeZone}</span><select aria-label="查看时间范围" value={calendar.period} onChange={(event) => calendar.setPeriod(event.target.value)}><option value="upcoming">即将到来</option><option value="day">当天</option><option value="week">7 天</option></select></div>
