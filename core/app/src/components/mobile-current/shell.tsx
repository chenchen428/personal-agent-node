...
export function MobileListShell({ section, activeAppId, title, note, children, query, setQuery, searchLabel, searchPlaceholder, filter, screenClassName }: {
  ...
  return <div className="mobile-current"><div className="mobile-stage"><div className={`phone${drawerOpen ? " menu-open" : ""}`} id="mobile-phone">
    ...
    {setQuery ? <SearchPanel
      ...
      onCancel={() => { setSearchDraft(""); setQuery(""); setSearchOpen(false); }}
    /> : null}
    <main className={`mobile-screen${screenClassName ? ` ${screenClassName}` : ""}`} data-mobile-scroll>{children}</main>
    {filter ? <FilterSheet open={filterOpen} close={() => setFilterOpen(false)} {...filter} /> : null}
    <MobileDrawer
      section={section}
...
function MobileHeader({ title, note, drawerOpen, openDrawer, searchOpen, openSearch, searchLabel, filter, filterOpen, openFilter }: {
  title: string;
  note: string;
  drawerOpen: boolean;
  openDrawer: () => void;
  ...
  return <header className={`mobile-header${filter ? " has-header-filter" : ""}`} data-primary-header hidden={searchOpen}>
    ...
    <div className="mobile-header-actions">
      ...
      {filter ? <button className={`header-filter-trigger${filter.value !== "all" ? " is-filtered" : ""}`} type="button" aria-expanded={filterOpen} onClick={openFilter}>{filter.value === "all" ? "筛选" : `${selected?.label} ${selected?.count}`}</button> : null}
    </div>
  </header>;
}

function SearchPanel({ open, label, placeholder, value, composing, onChange, onCommit, onCancel }: {
  open: boolean;
  label?: string;
  placeholder?: string;
  value: string;
  ...
  return <section className="list-search-panel" aria-label={label} hidden={!open}>
...
function FilterSheet({ open, close, label, description, value, setValue, options }: { open: boolean; close: () => void; label: string; description: string; value: string; setValue: (value: string) => void; options: FilterOption[] }) {
  useEffect(() => {
    if (!open) return;
  ...
  return <div className={`filter-sheet-layer${open ? " is-open" : ""}`} hidden={!open}>
    <button className="filter-sheet-backdrop" type="button" aria-label={`关闭${label}`} onClick={close} />
    <section className="filter-sheet" role="dialog" aria-modal="true" aria-label={label}>
      <span className="filter-sheet-handle" aria-hidden="true" />
      <header><div><strong>{label}</strong><span>{description}</span></div><button type="button" onClick={close}>取消</button></header>
      <div className="filter-sheet-options" role="radiogroup">
        {options.map((option) => <button className={option.value === value ? "is-active" : ""} type="button" role="radio" aria-checked={option.value === value} onClick={() => { setValue(option.value); close(); }} key={option.value}><span>{option.label}</span><small>{option.count} 个</small><i aria-hidden="true">✓</i></button>)}
      </div>
    </section>
  </div>;
}
