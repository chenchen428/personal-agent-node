...
export function MobilePages({ pageId = "" }: { pageId?: string }) {
  ...
  return <MobileListShell section="pages" title="发布页" note={note} query={query} setQuery={setQuery} searchLabel="搜索发布页" searchPlaceholder="搜索发布页" filter={{ label: "筛选发布范围", description: "选择要查看的页面范围", value: filter, setValue: setFilter, options }}>
    <div className="page-list-page">
      ...
      {query && filtered.length ? <SearchStatus query={query} count={filtered.length} /> : null}
      {!loading && !filtered.length ? <SearchEmpty title={query ? "没有找到相关页面" : "还没有发布页"} hint={query ? "试试页面标题或来源" : "PA 发布页面后会显示在这里"} /> : null}
      <div className="mobile-page-grid mobile-list-start">{filtered.map((item, index) => <PageCard page={item} index={index} key={item.id} />)}</div>
...
function PageCard({ page, index }: { page: PageItem; index: number }) {
  ...
  return <Link className="online-page-card" href={`/app/mobile/pages/${encodeURIComponent(page.id)}`}>
    <div className={`online-page-cover cover-${cover}`}><PageShot page={page} kind={cover} /></div>
...
function PageShot({ page, kind }: { page: PageItem; kind: string }) {
  if (page.thumbnailUrl) return <img src={page.thumbnailUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />;
