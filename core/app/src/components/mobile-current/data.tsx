...
export function activityKind(kind: string) { return ({ work: { label: "任务", icon: "↻" }, mail: { label: "邮件", icon: "@" }, page: { label: "发布页", icon: "▧" }, data: { label: "数据", icon: "▦" }, automation: { label: "自动化", icon: "⌁" } } as Record<string, { label: string; icon: string }>)[kind] || { label: "动态", icon: "◈" }; }
...
]; const assigned = new Set<string>(); const groups = definitions.map((definition) => ({ name: definition.name, skills: skills.filter((skill) => { const match = definition.test.test(`${skill.id} ${skill.name}`); if (match) assigned.add(skill.id); return match; }) })).filter((group) => group.skills.length); const remaining = skills.filter((skill) => !assigned.has(skill.id)); if (remaining.length) groups.push({ name: "发布与自动化", skills: remaining }); return groups; }
