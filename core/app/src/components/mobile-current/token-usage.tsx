...
export function MobileTokenUsageSection() {
  ...
  return <section className="mobile-about-section mobile-token-usage" aria-labelledby="mobile-token-title">
    ...
    {state !== "ready" ? <TokenUsageStatus state={state} detail={error} /> : usage ? <>
      ...
      <div className="mobile-token-total"><span>累计使用</span><strong>{formatCompactTokenCount(usage.totalTokens)}</strong><small>Tokens · {usage.sessionCount} 次会话 · {usage.requestCount} 次请求</small></div>
      <div className="mobile-token-breakdown"><div><span>输入</span><strong>{formatCompactTokenCount(usage.inputTokens)}</strong><small>Tokens</small></div><div><span>缓存</span><strong>{formatCompactTokenCount(usage.cachedInputTokens)}</strong><small>Tokens</small></div><div><span>输出</span><strong>{formatCompactTokenCount(usage.outputTokens + usage.reasoningOutputTokens)}</strong><small>Tokens</small></div></div>
