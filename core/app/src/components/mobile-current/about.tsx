...
export function MobileAbout() {
  ...
  return <MobileListShell section="about" title="关于" note="你的 Cove">
    <div className="mobile-about">
      {overview.loading ? <MobileAboutMachineSkeleton /> : <section className="mobile-about-machine"><span className="mobile-about-mark"><CoveMark title="Cove" /></span><div><span>当前机器</span><h1>{overview.value?.machine.id || "本机"}</h1><p>本机运行 · 最近检查：刚刚</p></div><em>{overview.value?.machine.state === "running" ? "在线" : "检查中"}</em></section>}
