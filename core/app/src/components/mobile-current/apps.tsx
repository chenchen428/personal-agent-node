...
export function MobileApps() {
  ...
  return <MobileListShell section="apps" title="全部应用" note={`${apps.length} 个可用应用`}>
    <section className="mobile-app-directory">
      ...
      {loading && !apps.length ? <MobileContentSkeleton kind="apps" /> : <div className="mobile-app-list">{apps.map((app, index) => <Link className={`mobile-app-card${index === 0 ? " featured" : ""}`} href={app.mobileRoute || app.route} prefetch key={app.id}><span className={`mobile-app-icon${index % 3 === 1 ? " mint" : index % 3 === 2 ? " coral" : ""}`}>{app.name.slice(0, 1)}</span><div><strong>{app.name}</strong><p>{app.description || "由 Cove 安装在这台电脑上的应用。"}</p><small>{appSurfaces(app)}</small></div><i>打开</i></Link>)}</div>}
