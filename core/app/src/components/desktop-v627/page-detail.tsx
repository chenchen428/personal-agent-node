...
export function PageDetail({ pageId }: { pageId: string }) {
  ...
  return <div className={`runtime-page runtime-page-full runtime-theme-${page.headerTheme}`}><header className="runtime-bar"><Link href="/app/pages">← 发布页</Link><code>{page.shareUrl || page.url}</code><div className="runtime-page-actions">{page.visibility === "public" ? <button type="button" onClick={() => void navigator.clipboard.writeText(page.shareUrl || page.url)}>分享</button> : <span className="pa-status private">私有</span>}</div></header><iframe src={page.url} title={page.title} scrolling="yes" /></div>;
}
