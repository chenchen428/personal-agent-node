...
import { Badge, Card, SettingRow } from "../desktop-v72/primitives";
...
import { DailyTokenLimitSetting } from "./daily-token-limit-setting";
...
export function SettingsPage() {
  ...
  return <SettingsLayout active="general"><div className="settings-inner"><h1>通用</h1><p>管理当前空间中的 Cove 行为。桌面客户端始终作为本机可信入口。</p>
    ...
    <h2 className="settings-section-title">访问安全</h2><Card className="setting-group"><SettingRow title="这台电脑" description="桌面客户端无需登录，直接进入本机工作区" control={<Badge tone="success">本机可信</Badge>} /><SettingRow title="公网访问密码设置" description="用于公网访问登录，修改后其他设备的登录状态失效" control={<Button variant="outline" onClick={() => setDialog("password")}>修改密码</Button>} /><AuthorizationModeSetting /></Card>
    <h2 className="settings-section-title">数据导出</h2><Card className="setting-group"><SettingRow title="个人工作区" description="Workspace、邮件、技能与应用数据保存在用户选择的目录" control={<Button variant="outline" onClick={() => setDialog("workspace")}>查看</Button>} /><SettingRow title="导出数据" description="将邮件、发布页、历史规划与 SQLite 数据库打包为 ZIP 文件" control={<DataExportControl onFeedback={setFeedback} />} /><SettingRow title="客户端版本" description="桌面端、Agent 与 Core 同步更新" control={<Badge>{runtime.value?.version || "读取中"}</Badge>} /></Card>
