...
export function PageThumbnail({ page }: { page: PageItem }) { const className = `page-shot ${page.headerTheme === "dark" ? "page-shot-travel" : "page-shot-finance"}`; const thumbnailUrl = page.desktopThumbnailUrl || page.thumbnailUrl; return thumbnailUrl ? <div className={className}><img src={thumbnailUrl} alt={page.desktopThumbnailAlt || page.thumbnailAlt || `${page.title} 页面预览`} /></div> : <div className={className} role="img" aria-label={`${page.title}的静态缩略图`}><span>{page.visibility === "public" ? "PUBLIC PAGE" : "PRIVATE PAGE"}</span><strong>{page.title}</strong><p>{page.summary}</p></div>; }
...
export function useJson<T>(url: string) { return useClientResource<T>(url); }
...
export function desktopActivityHref(item: ActivityItem) { if (item.kind === "conversation") return "/app/conversations"; if (item.kind === "work") return "/app/workers"; if (item.kind === "mail") return "/app/mail"; if (item.kind === "page") return "/app/pages"; return "/app/data"; }
