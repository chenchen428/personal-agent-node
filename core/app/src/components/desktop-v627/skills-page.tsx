...
export function SkillsPage() {
  const { value, loading, error } = useJson<SkillsResponse>("/api/skills");
...
function SkillDetail({ skill, category, spaceName, bypass }: { skill: Skill; category: string; spaceName: string; bypass: boolean }) {
  ...
  return <div className="skill-readonly-detail">
    <header><div><span>{category}</span><h2>{skill.name}</h2><p>{spaceName} · 当前空间可用</p></div><Badge tone="success">可使用</Badge></header>
