...
export function DomainBindingSop({ kind, verification, remainingSeconds, authorizationUrl, collapsed, onToggle }: { kind: "mail" | "sites"; verification: DomainVerification; remainingSeconds: number; authorizationUrl?: string; collapsed: boolean; onToggle: () => void }) {
  ...
  const authorizePanel = error || <div className="domain-human-guide" role="status"><strong>正在后台确认平台连接</strong><p>Cove 会使用当前已登录的 Owner 会话完成连接，不需要手动确认；静默连接失败后才会显示恢复入口。</p>{authorizationUrl ? <small>后台授权页面已准备，正在等待自动回调。</small> : <small>正在准备后台授权。</small>}</div>;
  ...
  const verifyPanel = error || (kind === "mail" ? <div className="domain-provider-note"><strong>公开测试邮件</strong><span>Cove 会把分配的收件地址提交给 TestEmailSender，由 JoltMx 发送固定测试邮件。继续表示你确认有权测试该地址并同意其 <a href="https://testemailsender.com/terms" target="_blank" rel="noreferrer">服务条款</a>与<a href="https://testemailsender.com/privacy" target="_blank" rel="noreferrer">隐私政策</a>。</span></div> : <div className="domain-human-guide" role="status"><strong>正在发布验证 Page</strong><p>Cove 会生成带有本次标记的页面，并从最终公网地址回读核对。</p></div>);
