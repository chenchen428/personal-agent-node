...
export function PagesPage() {
  ...
  return <PageSurface><PageHeader title="发布页" description="PA 交付的完整网页。列表直接预览页面内容，打开后可查看完整页面。" actions={<>{searchOpen ? <SearchField autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索发布页…" aria-label="搜索发布页" /> : <button className="icon-button" type="button" aria-label="搜索" onClick={() => setSearchOpen(true)}><Search size={16} /></button>}<SegmentedControl value={visibility} onChange={setVisibility} options={[{ label: "全部", value: "all" }, { label: "私有", value: "private" }, { label: "公开", value: "public" }]} /></>} />
    {loading && !value ? <LoadingState label="正在读取发布页" /> : <section className="gallery">{pages.map((page) => <a className="card gallery-card" href={page.url || page.shareUrl} target="_blank" rel="noreferrer" aria-label={`在默认浏览器中打开${page.title}`} key={page.id}><PagePreview page={page} /><div className="gallery-copy"><h2>{page.title}</h2><p>{page.summary}</p><div className="gallery-meta"><Badge tone={page.visibility === "public" ? "info" : undefined}>{page.visibility === "public" ? "公开" : "私有"}</Badge><span className="row-trailing">{relativeTime(page.updatedAt)} <ArrowUpRight size={12} style={{ display: "inline" }} /></span></div></div></a>)}</section>}
    {!loading && !pages.length ? <IllustratedEmptyState className="empty-state" variant="pages" title={query ? "没有找到相关发布页" : "还没有发布页"} description={query ? "换个关键词，或切换可见范围后再试。" : "主 Agent 发布网页后，会在这里显示并打开页面。"} /> : null}
  </PageSurface>;
}
