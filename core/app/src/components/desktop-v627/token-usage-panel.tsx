...
import { formatCompactTokenCount, formatTokenUpdatedAt } from "../token-usage/format";
...
export function TokenUsagePanel({ range, setRange, usage, loading, error }: {
  ...
  return <section className="desktop-token-panel" aria-labelledby="desktop-token-title">
    ...
    {state !== "ready" ? <TokenUsageStatus state={state} detail={error} /> : usage ? <>
      <div className="desktop-token-overview"><div className="desktop-token-total"><span>累计使用</span><strong>{formatCompactTokenCount(usage.totalTokens)}</strong><small>Tokens · {usage.sessionCount} 次会话 · {usage.requestCount} 次请求</small></div><div className="desktop-token-cache"><span><Database aria-hidden="true" />缓存输入占比</span><strong>{usage.cacheRate}%</strong><small>重复上下文优先复用缓存</small></div></div>
      <div className="desktop-token-metrics">{metrics.map((metric) => <div className={metric.tone} key={metric.label}><span>{metric.label}</span><strong>{formatCompactTokenCount(metric.value)}</strong><small>Tokens</small></div>)}</div>
