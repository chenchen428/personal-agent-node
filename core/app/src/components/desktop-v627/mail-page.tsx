...
export function MailPage() {
  ...
  return <main className="v72-page v72-page-flush">
    <div className="v72-split-view">
      <aside className="v72-split-list" aria-label="邮件列表" aria-busy={loading}>
        ...
        {loading && !view ? <LoadingState label="正在读取邮件" compact /> : events.map((item) => <button className={`v72-select-row${selectedId === item.id || selected?.id === item.id ? " selected" : ""}`} type="button" onClick={() => { setSelectedId(item.id); void load(item.id); }} key={item.id}>
        ...
        {!loading && !events.length ? <div className="v72-empty">{error || "暂时没有邮件"}</div> : null}
      </aside>
      <article className="v72-split-detail" aria-busy={loading}>{loading ? <LoadingState label="正在准备邮件内容" /> : selected ? <div className="v72-detail-wrap">
