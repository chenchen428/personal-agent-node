    ...
    <div className="mail-workspace-next">
      ...
      <article className="mail-reader-next">
        {view.selectedEvent && view.content ? <>
          ...
          <div className={`mail-agent-result ${matchedRun ? "is-matched" : ""}`}><CheckCircle2 className="size-4" /><div><strong>{matchedRun ? "Agent 已关注" : "已安全归档"}</strong><span>{matchedRun?.reason || "邮件已经保存在本机，当前没有自动化规则需要处理。"}</span></div>{matchedRun?.sessionId ? <Link href={`/app/workers?task=${encodeURIComponent(matchedRun.sessionId)}`}>查看处理</Link> : null}</div>
