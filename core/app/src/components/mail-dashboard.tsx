    ...
    <div className="mail-workspace-next">
      ...
      <article className="mail-reader-next">
        {view.selectedEvent && view.content ? <>
          ...
          <div className={`mail-agent-result ${matchedRun ? "is-matched" : ""}`}><CheckCircle2 className="size-4" /><div><strong>{matchedRun ? "已创建任务" : "已安全归档"}</strong><span>{matchedRun?.reason || "邮件已经保存在本机，当前无需创建任务。"}</span></div>{matchedRun?.sessionId ? <Link href={`/app/workers?task=${encodeURIComponent(matchedRun.sessionId)}`}>查看任务</Link> : null}</div>
