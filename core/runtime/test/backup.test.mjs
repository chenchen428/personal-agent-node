...
import { createEncryptedBackup, restoreEncryptedBackup, verifyEncryptedBackup } from "../src/backup.ts";
...
test("restores a protected backup with a new replacement Node identity", async () => {
  ...
  fs.mkdirSync(path.join(dataRoot, "plugins", "example"), { recursive: true });
  fs.writeFileSync(path.join(dataRoot, "plugins", "example", "personal-agent.plugin.json"), "{}");
  ...
  try {
    ...
    assert.equal(fs.readFileSync(path.join(target, "plugins", "example", "personal-agent.plugin.json"), "utf8"), "{}");
