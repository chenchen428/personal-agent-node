...
export async function setupStatus({
  ...
  const checks = [
    ...
    makeCheck('installation.console-auth', localAuthReady, localAuthDocument?.verifier ? '远程访问密码已使用不可逆校验器' : localAuthReady ? '请设置自己的远程访问密码' : '需要设置远程访问密码', { configured: localAuthReady, durableVerifier: Boolean(localAuthDocument?.verifier), localClientPasswordRequired: false }, generatedAt, remoteSelected ? (localAuthDocument?.verifier ? undefined : 'action-required') : 'not-selected'),
