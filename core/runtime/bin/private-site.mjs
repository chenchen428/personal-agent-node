...
function seedPublications(config) {
  const seeds = [
    [path.join(workspaceRoot, "workspace", "publications", "blog"), path.join(config.dataRoot, "publications", "blog")],
    [path.join(workspaceRoot, "workspace", "publications", "docs"), path.join(config.dataRoot, "publications", "docs")],
    [path.join(workspaceRoot, "workspace", "publications", "resources"), path.join(config.dataRoot, "publications", "resources")],
  ];
...
async function importLegacyDataCommand() {
  ...
  const { importLegacyData } = await import("../src/migration.ts");
...
async function backupCommand() {
  ...
  const { createEncryptedBackup } = await import("../src/backup.ts");
  const supervisor = readSupervisor(config);
...
async function restoreVerifyCommand() {
  ...
  const config = resolveNodeConfig();
  const { verifyEncryptedBackup } = await import("../src/backup.ts");
