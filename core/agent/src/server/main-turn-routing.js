...
export function buildPageTemplateTask({ request, template }) {
  ...
  return [
    ...
    `必须使用模板指定生成器 ${template.implementation.generator}，并校验产物标识 ${template.implementation.artifactMarker}、模板 ID、版本与契约摘要 ${template.contractDigest}。`,
    "必须通过 pa-cli pages publish --template 发布；CLI 会在上传前复核模板标识、ID、版本和产物哈希，并把模板溯源写入 Page 清单。",
