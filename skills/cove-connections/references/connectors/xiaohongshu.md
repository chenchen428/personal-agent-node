...
| `open` | R1 | 在当前Space绑定的浏览器会话打开固定小红书官方登录页面；此动作不要求预先登录，不自动填写或提交账号。 |
| `search` | R0 | 按关键词搜索笔记。 |
