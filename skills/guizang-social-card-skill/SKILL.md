...
Generated work must live in a task folder, not in the skill root. Default to `publications/<slug>/` in the customer Workspace, or use the explicit output folder requested by the user. Do not create root-level task folders such as `social-card-*`, `livephoto-*`, `wechat-*`, `output/`, or loose rendered assets next to `SKILL.md`.
...
- Create a task folder under `publications/<slug>/` by default, or inside the user-requested output folder. Never put generated task folders, rendered images, MOV files, `.pvt` packages, or downloaded sources in the skill root next to `SKILL.md`.
...
- Keep `node skills/guizang-social-card-skill/scripts/validate-social-deck.mjs <task-dir>` available for auto-check passes. It checks overflow (R1), footer collision (R2), Swiss bold display (R3), minimum font size (R4), 4-band density (R5), `.h-xl` line caps (R6), browser-default figure margin drift (R7), visual bounds / bottom whitespace (R8), and title-to-content gaps (R9). The validator requires Playwright; if it is unavailable, report that clearly and use the available browser/screenshot workflow instead. Do not install dependencies without the user's approval. Exit code 1 on any FAIL — fix before final delivery when auto-check is requested. WARN is advisory but read it.
...
Policy: **verify rights before download and preserve provenance.** Prefer user-supplied assets, public-domain material, or sources whose license permits the requested use. Do not assume that composing an image transfers rights to the user. Record the source URL, creator when known, license, and retrieval date; ask the user before using material whose rights are unclear.
...
4. If the user says "你查吧" / "auto-check" / "yes" — only then run `node skills/guizang-social-card-skill/scripts/validate-social-deck.mjs <task-dir>`, fix any FAIL, and re-render before final delivery. Mention density/cap WARNs.
...
- Never create generated work in the skill root. All task artifacts must be under `publications/<slug>/` by default, or under a user-requested output folder. Root-level generated folders like `social-card-*`, `livephoto-*`, `wechat-*`, and loose output assets are forbidden.
