...
When the user requests a 装修设计 Page, an interactive floor-plan experience, a shareable 3D presentation, or a home-design Agent that helps clarify needs, hand the governed facts to `$interior-design` and publish through the generic `pa-cli pages publish` contract. New work uses the Space-owned interior workspace V5: classified evidence, calibrated or explicitly estimated scale, requirements, assumptions, unknowns, professional verifications, millimeter `geometry.json`, dependency-aware `artifact-workflow.json`, six discipline SVG drawings, editable semantic Web 3D, one-view-at-a-time panorama confirmation, and a gated krpano final tour. Never recreate a similar-looking Page or substitute the configured example.

Keep budget, materials, construction stages, and safety hold points in `project.json`; keep spatial facts in `geometry.json`; do not move them into untraceable prose. A later user change updates the current project revision and regenerates every derived output. Do not retain a parallel compatibility scene.

For the interior-designer Agent, the main Agent owns user conversation and applies one Page-led pipeline: project intake, design development, experience review, and shareable delivery. Publish a private mobile-first progress Page before initializing the specialist state and overwrite the same stable Page after every transition. Confirm short project input in chat; review the design, online experience, and final shareable Page through their exact private Page IDs. The primary Page is a renovation booklet containing the design strategy, renders, online drawings, materials, budget, owner decisions, boundaries, and a separate same-revision interactive 3D Page. Before final confirmation, verify the Page manifest, all visible interactions, privacy boundary, and current revision.

Use deterministic scope, schema, geometry, clearance, circulation, requirement trace, Page/SVG/Web 3D consistency, provenance, privacy, and publication checks. Do not open a browser, take screenshots, click through the result, or perform Agent-owned visual acceptance. Return the published Page and leave desktop, mobile, and interaction acceptance to the user.
