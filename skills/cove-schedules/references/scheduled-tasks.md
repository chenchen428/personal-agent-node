...
Convert ordinary reminder language into a five-field cron expression and preserve the user's stated IANA timezone. If the user gives a local clock time without a timezone, use the current Cove timezone and state it in the confirmation. The scheduler rejects expressions that may run more often than once every fifteen minutes.
