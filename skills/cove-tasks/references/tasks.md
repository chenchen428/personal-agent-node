...
Resume only when the selected task is paused and uniquely matches the user's intended work:

```bash
pa-cli session resume \
  --session <task-id> \
  --task "继续完成原任务；先检查已有进展和暂停原因，再从未完成处继续。"
```

Never resume a task merely to answer a status question. Never resume a different task because its title or keywords look similar. After resuming, query `session status` and report the persisted state.
