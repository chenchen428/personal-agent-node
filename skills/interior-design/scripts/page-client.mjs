...
let currentView = initialView, currentRoom = '', displayMode = 'overall', lightMode = model.lighting.mode || 'day', runtime;
...
document.querySelector('#light').addEventListener('click', () => { lightMode = lightMode === 'day' ? 'evening' : 'day'; document.querySelector('#light').textContent = lightMode === 'day' ? '日间' : '傍晚'; runtime.setLighting(lightMode); });
document.querySelector('#reset').addEventListener('click', () => { displayMode = 'overall'; applyView('iso', ''); syncMode(); });
applyView(initialView, ''); syncMode();
...
function syncMode() { document.querySelectorAll('[data-mode]').forEach((button) => button.classList.toggle('active', button.dataset.mode === displayMode)); }
...
function startWebGL(canvas) {
  ...
  const bounds = model.project.bounds || calculateBounds(model.rooms), center = new THREE.Vector3((bounds.maxX + bounds.minX) / 2, 0, (bounds.maxZ + bounds.minZ) / 2);
  ...
  if (model.presentation?.reveal && !matchMedia('(prefers-reduced-motion: reduce)').matches) {
    ...
    const duration = Math.max(400, Math.min(5000, model.presentation.revealDurationMs || 1600));
  ...
  return { setLighting, update(view, roomId, mode) {
    ...
    const pose = viewPose(view, roomId, center, span), direction = pose.position.clone().sub(pose.target);
...
function viewPose(view, roomId, center, wholeSpan) {
  const room = model.rooms.find((entry) => entry.id === roomId), roomBounds = room ? calculateBounds([room]) : null;
  ...
  if (view === 'top') return { target, position: target.clone().add(new THREE.Vector3(0.01, span * 1.58, 0.01)) };
