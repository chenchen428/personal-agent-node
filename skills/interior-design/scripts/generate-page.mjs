...
<section class="stage"><nav class="rooms" aria-label="浏览空间"><span>空间索引</span><button class="active" type="button" data-room=""><b>00</b><strong>整体方案</strong><small>原 C 户型与客厅 6m 挑空</small></button>${roomButtons}</nav><div class="viewport"><canvas id="scene" aria-label="${title}，可旋转、平移和缩放的三维概念户型"></canvas><span class="gesture">拖动旋转 · 滚轮或双指缩放 · 右键平移</span><div class="level-tools" role="group" aria-label="切换楼层与剖切模式"><button class="active" type="button" data-mode="overall">整体轴测</button><button type="button" data-mode="lower">下层</button><button type="button" data-mode="upper">上层</button><button type="button" data-mode="section">挑空剖切</button><button type="button" data-mode="free">自由查看</button></div></div>
...
<aside id="fallback" hidden><strong>兼容投影模式</strong><span>当前设备未启用 WebGL，仍可拖动与缩放查看分层关系。</span></aside></div>
<section id="orientation-gate" class="orientation-gate" role="dialog" aria-modal="true" aria-labelledby="orientation-title" aria-describedby="orientation-copy" hidden><div class="orientation-dialog"><svg class="orientation-icon" viewBox="0 0 64 64" aria-hidden="true"><rect x="21" y="12" width="22" height="40" rx="4"></rect><path d="M13 24a22 22 0 0 1 34-13"></path><path d="m46 5 2 8-8-1"></path></svg><p class="orientation-kicker">3D 浏览建议</p><h1 id="orientation-title">旋转手机以横屏查看完整户型</h1><p id="orientation-copy">横屏可完整显示楼层切换、6 米挑空和自由浏览控件。</p><p id="orientation-status" class="orientation-status" aria-live="polite"></p><div class="orientation-actions"><button id="request-landscape" type="button">尝试进入横屏</button><button id="continue-portrait" class="secondary" type="button">仍以竖屏查看</button></div></div></section>
<script id="model" type="application/json">${safeModel}</script><script>${script}</script></main></body></html>`;
