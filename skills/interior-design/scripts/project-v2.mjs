...
export function importV1Seed(model) {
  const bounds = model.project?.bounds || boundsForPoints(model.rooms.flatMap((room) => room.polygon));
  const footprint = [[bounds.minX, bounds.minZ], [bounds.maxX, bounds.minZ], [bounds.maxX, bounds.maxZ], [bounds.minX, bounds.maxZ]];
  const edgeForWall = (wall) => footprint.findIndex((point, index) => sameSegment(wall.from, wall.to, point, footprint[(index + 1) % footprint.length]));
  const materials = new Map((model.materials || []).map((material) => [material.id, material]));
  const requirements = (model.qualityReview?.requirementTrace || ['Preserve the imported legacy concept intent.']).map((summary, index) => ({
    requirementId: `req-legacy-${index + 1}`,
    source: 'legacy-v1-quality-review',
    summary,
    priority: 'must',
    status: 'satisfied',
    sceneNodeIds: [],
    verification: { method: 'legacy-record', result: 'imported' },
  }));
  return {
    title: model.project?.title || 'Imported renovation concept',
    status: 'selected_concept',
    evidence: [{
      evidenceId: 'evidence-legacy-v1',
      relativePath: 'evidence/legacy-v1-model.json',
      ...
      allowedUses: ['structure', 'edit'],
      observations: ['Imported from Personal Agent interior-design schema v1.'],
      inferences: [],
      redactionStatus: 'private-only',
      contentHash: sha256(canonicalJson(model)),
    }],
    brief: {
      household: [],
      scope: ['layout', 'furniture', 'materials'],
      budget: { currency: 'CNY', totalMinor: 0, confidence: 'unknown' },
      schedule: { confidence: 'unknown', phases: [] },
      requirements,
    },
    assumptions: [{ assumptionId: 'assumption-legacy-scale', summary: 'Legacy geometry and scale are preserved without claiming survey precision.', confidence: 'estimated' }],
    unknowns: [{ unknownId: 'unknown-site-verification', summary: 'Site measurements and hidden services were not part of the v1 model.' }],
    professionalVerifications: [{ verificationId: 'verify-site', category: 'site-measurement', status: 'required', summary: 'Verify all construction dimensions on site.' }],
    concepts: [{
      conceptId: 'concept-imported',
      name: 'Imported v1 concept',
      summary: 'Non-destructive migration of the existing concept model.',
      singleOptionReason: 'The migration preserves the existing accepted concept rather than inventing an alternative.',
      tradeoffs: ['Preserves the legacy layout; professional project metadata is added without changing geometry.'],
      budgetItems: [],
      levels: [{
        levelId: 'ground',
        name: 'Ground level',
        elevation: 0,
        height: Math.max(...model.rooms.map((room) => room.height || 2.8), 2.8),
        footprint,
        rooms: model.rooms.map((room) => ({
          roomId: room.id,
          name: room.name,
          kind: inferRoomKind(room.name),
        ...
        walls: model.walls.map((wall) => ({
          ...
          exteriorEdge: edgeForWall(wall),
        })),
        openings: model.openings.map((opening) => ({
          openingId: opening.id,
          type: opening.kind,
          wallId: opening.wallId,
          position: opening.offset,
          width: opening.width,
          height: opening.height,
          sillHeight: opening.kind === 'window' ? opening.sillHeight || 0.9 : 0,
          connectsRoomIds: opening.connectsRoomIds || [],
        })),
        items: model.furniture.map((item) => ({
          itemId: item.id,
          ...
          color: materials.get(item.material)?.color || '#d8c9b6',
          clearanceExempt: item.clearanceExempt === true,
        })),
        stairs: [],
        guardrails: [],
        voids: [],
    ...
    selectedConceptId: 'concept-imported',
    designIntent: {
      style: ['legacy-v1-import'],
      materials: [...materials.values()].map((material) => ({ materialId: material.id, name: material.name, color: material.color, maintenance: 'verify-product-specification' })),
      lighting: [model.lighting || { mode: 'day' }],
      maintenance: ['Confirm final material samples and maintenance instructions before procurement.'],
    },
    decisions: [{ decisionId: 'decision-import', summary: 'Preserve legacy layout during v2 migration.', rationale: 'Migration must be non-destructive.', requirementIds: requirements.map((item) => item.requirementId) }],
  };
}
...
function sameSegment(a, b, c, d) { return (samePoint(a, c) && samePoint(b, d)) || (samePoint(a, d) && samePoint(b, c)); }
...
function inferRoomKind(name = '') {
  if (/bed|卧/i.test(name)) return 'bedroom';
  if (/kitchen|厨/i.test(name)) return 'kitchen';
  if (/bath|卫/i.test(name)) return 'bathroom';
  if (/entry|玄关/i.test(name)) return 'entry';
  return 'living';
}
