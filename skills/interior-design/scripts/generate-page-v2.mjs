...
export function generateProfessionalPage({ projectDir: projectDirInput, context, output, skillRoot, delivery = loadInteriorDeliveryContract(skillRoot) }) {
  ...
  const conceptRender = loadProjectConceptRender(projectDir, project);
  ...
  const renderPanel = conceptRender ? renderConceptPanel(conceptRender) : '';
  const evidencePanel = conceptRender
...
<article class="presentation-panel document-panel" data-presentation-panel="requirements" hidden><header><div><small>REQUIREMENT TRACE</small><h2>需求与专业边界</h2></div><p>点击有模型关联的需求，可在 3D 场景中定位对应构件。</p></header><div class="document-grid"><section class="card"><div class="card-head"><strong>需求状态</strong></div><div class="requirement-list">${requirements}</div></section><aside class="card"><div class="card-head"><strong>假设、未知与专业核验</strong></div><div class="stack">${assumptions}</div></aside></div></article>`;
  const presentationNavigation = conceptRender
    ? `<nav class="presentation-switch" aria-label="装修设计资料切换"><button type="button" data-presentation="requirements" aria-pressed="false">${ICONS.requirements}用户需求</button><button class="active" type="button" data-presentation="model" aria-pressed="true">设计稿</button><button type="button" data-presentation="render" aria-pressed="false">${ICONS.render}渲染稿</button></nav>`
...
function loadProjectConceptRender(projectDir, project) {
  const evidence = project.evidence.find((entry) => entry.classification === 'concept-render'
    && entry.relativePath
    && ['redacted', 'not-required'].includes(entry.redactionStatus)
    && entry.allowedUses?.includes('delivery'));
  ...
  return loadProjectPlanAsset(projectDir, evidence, '基于当前 SU 设计稿生成的概念渲染稿');
}
