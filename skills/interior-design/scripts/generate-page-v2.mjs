...
import { resolveInteriorStyleGuide } from './interior-style-contract.mjs';
...
export function generateProfessionalPage({ projectDir: projectDirInput, context, output, skillRoot, delivery = loadInteriorDeliveryContract(skillRoot) }) {
  ...
  const styleGuide = resolveInteriorStyleGuide(project, scenePayload);
  pagePayload.styleGuide = styleGuide;
  ...
  const stylePicker = `<label class="style-control"><span>风格预览</span><select class="style-picker" data-style-profile aria-label="切换 3D 风格预览">${styleGuide.options.map((entry) => `<option value="${escapeAttr(entry.styleId)}"${entry.styleId === styleGuide.selection.selectedStyleId ? ' selected' : ''}>${escapeHtml(entry.label)}</option>`).join('')}</select></label>`;
  const threeDHtml = renderThreeDPage({ delivery, displayTitle, fallbackSvg, pagePayload, subtitle, title, viewer, viewerStyle, conceptPicker, stylePicker, levelButtons, allLevelButton, concept });
  ...
  fs.writeFileSync(path.join(staging, 'style-guide.json'), `${JSON.stringify(styleGuide, null, 2)}\n`, { mode: 0o600 });
  fs.writeFileSync(path.join(staging, 'audit.json'), `${JSON.stringify(sanitizeAudit(audit, pageMappings), null, 2)}\n`, { mode: 0o600 });
  const files = ['index.html', '3d/index.html', 'scene.json', 'style-guide.json', 'audit.json'];
  ...
  const manifest = {
    ...
    style: {
      guide: 'style-guide.json',
      selectedStyleId: styleGuide.selection.selectedStyleId,
      status: styleGuide.selection.status,
      effectRenderBinding: 'same-style-id-required',
    },
...
function renderThreeDPage({ delivery, displayTitle, fallbackSvg, pagePayload, subtitle, title, viewer, viewerStyle, conceptPicker, stylePicker, levelButtons, allLevelButton, concept }) {
  return `<!doctype html>
