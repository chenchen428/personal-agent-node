...
8. Read [delivery-v2.md](references/delivery-v2.md), confirm the user-uploaded source plan and Agent-uploaded revision annotation are redacted delivery images, and invoke the separate `$render-interior-pages` capability. Maintain a consistency matrix from requirement and decision through scene node, governed material or budget scope, camera/view, and acceptance status. Supply governed project data only; never author or patch Page HTML, CSS, JavaScript, responsive logic, or 3D controls:

   `node skills/render-interior-pages/scripts/cli.mjs render --project-dir <project-dir> --output <project-dir>/derived/page --json`

   If the style direction is not confirmed, run `node skills/render-interior-pages/scripts/cli.mjs styles --json`, explain suitable directions, and write exactly one selected `styleId` to `demandWorkflow.styleProfile.primary` before rendering. Read the emitted `style-guide.json`. Its single selected `styleId` is one contract shared by the 3D palette, PBR material families, lighting/exposure, soft-furnishing language, the one-image style sample, and all later effect-render prompts. The delivered Page has no style selector or client-side switching. Style feedback must update `demandWorkflow.styleProfile.primary`, mark older render records stale, compile a new revision, rerender and complete the renderer's Agent review before generating effect renders. Every effect-render record must carry the same `styleId` plus the manifest `styleGuideSha256`; a mismatch blocks delivery.
   ...
   Read the renderer's `agent-review.json`, inspect all five required views and the one rendered selected style, verify the shared effect-render binding, record observations using agent-review v2, and run its `review` command. Any problem reopens governed project data and requires a new render; never fix generated Page code directly.
