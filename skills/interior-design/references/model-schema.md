...
- For a strictly single-level concept, use `assertions.singleLevelOnly: true` and `maxRoomHeight`. Validation then rejects multiple levels and any slab, void, stair, or railing collection with content.
- `views[]` defines review modes with `id`, `label`, resolved `roomIds`, and optional `showCirculation`. `circulationPaths[]` contains `id`, `name`, two or more plan points, and a six-digit color. Use these for functional-zone and movement review without changing source geometry.
