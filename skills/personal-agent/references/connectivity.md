...
After GitHub Owner OAuth, Cloud requires a separate CLI password. Bind the redacted resource view with `personal-agent cloud login --github-user-id <numeric-id> --password-stdin --json`; never accept a password argument. Managed mail and managed configuration remain disabled until the returned public domain and matching Agent mail identity both validate. A WeChat user may start the dedicated one-time flow with `云账号绑定 <GitHub数字用户ID>`; only the immediately following message is treated as the password, intercepted before Agent conversation persistence, and discarded after authentication.
