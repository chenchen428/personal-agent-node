...
The canonical main Agent may attach validated current-Space managed images or safe files to the ordinary reply for this inbound conversation. The orchestrator strips the versioned final-reply envelope, sends text first, then calls Qianxun `sendImage` or `sendFile` in order, and records structured delivery receipts. This is limited to the already authorized conversation recipient and is not the manual R2 send command. Workers only report artifact object IDs and never send attachments. See [final reply attachments](../final-reply-attachments.md).
