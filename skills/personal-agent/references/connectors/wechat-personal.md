...
千寻 Pro 的 `recvMsg` 回调仅接受本机请求，并绑定配置时确认的微信账号。当前实例的完整回调地址由页面从本机服务读取并提供复制，不应根据浏览器端口手工猜测。连接器先进行去重和消息规范化，再把每个私聊或群聊的消息写入本机会话历史；策略拒绝、自己发送和非文本消息也保留，但不会触发主 Agent。只有通过判断的文本消息才进入主 Agent，并自动附带同一会话在当前消息之前最多 100 条历史作为不可信上下文。主 Agent 的回执和回复通过同一个千寻连接器发回原私聊或群，不会误用微信 claw。
