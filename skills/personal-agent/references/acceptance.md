...
- Fresh installation must start the embedded backend and open trusted direct-loopback `/app/setup` without a password prompt. The user establishes a durable salted password verifier only when remote or tunnel access is enabled. Setup Center, `personal-agent setup status` and doctor share one versioned contract and independently report console, Agent, remote and mail readiness.
- With Cloud disconnected, verify passwordless direct-loopback `/app`, local Web conversation, BYOK, channels, managed platforms, files, automation, Pages and encrypted backup/restore. For release and final acceptance, install the public GitHub Release asset, use its direct-loopback `/app/chat` without a password prompt, send a unique prompt to the real Agent runtime and observe the Agent reply in that same session. In the evidence below, `authenticated=true` means the gateway authorized the trusted direct-loopback request; it does not mean the user entered a local password. Use exactly this sanitized object and never record the prompt, reply or session identifier:
...
- Verify desktop and mobile Console use, public/authenticated/local-admin/internal route classes and default denial for unknown routes. Direct loopback `authenticated` routes bypass the password, reverse-tunnel and non-loopback `authenticated` routes require it, and `local-admin` accepts direct loopback only.
...
Managed Cloud is not a prerequisite for the Node core gate. For an integrated customer journey, additionally verify the remote access password, browser device authorization, same-origin verification URLs, bounded polling/slow-down, expiry and denial, single-use enrollment credentials, recoverable pending enrollment, device enrollment, heartbeat, tenant assignment and managed data plane. Verify CLI/browser output never contains a device code, enrollment credential, Node token, generated migration password or tunnel secret.
