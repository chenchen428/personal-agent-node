...
For R2 and R3, create a plan that returns an operation ID, impact summary, expiry and confirmation digest. The user approves that exact digest through the trusted direct-loopback local console or an interactive local TTY. Then execute the approved operation and verify it.
