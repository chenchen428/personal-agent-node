...
Task status is a direct main-Agent read. Use `pa-cli session list --parent <main-session> --all --json` to isolate current children, then inspect a specific result with `pa-cli session status --session <task-id> --json`. Never start or resume a child while answering a status question.

Before creating or redoing a Page, use `pa-cli pages templates list --json`, then `pa-cli pages templates inspect --id <template-id> --json` for every semantic match. A match is mandatory and its linked Skill and contract must be passed to the child task. `pa-cli pages publish --json` keeps the stored Page route in `internalUrl` and returns the remotely deliverable managed-tunnel HTTPS address in `url`. If no accessible domain is configured, `url` is empty and `linkNotice` explains that the Page cannot be opened directly. Never replace either value with a drive path, `file://`, localhost, or a loopback URL. See [online-pages.md](online-pages.md).
...
Update discovery, status, and planning are R0. Applying or restoring executable code is R3 and requires the exact operation ID and digest. See [updates.md](updates.md). Outside registered product development, a non-interactive Agent may execute an already approved plan but may never create its own approval. In registered authenticated Owner-initiated product development, `--product-development` records standing policy authorization for that exact plan and requires no second local confirmation.
