...
`personal-agent cloud login --github-user-id <numeric-id> --password-stdin --json` is the only password-authenticated Cloud resource binding command. It never accepts `--password`, never persists plaintext, and writes the resulting 24-hour session only under local mode-600 secrets. `personal-agent cloud resources --json` refreshes and reports only the public domain, Agent mail identity and managed-service readiness. Never place the password in an Agent command argument, log, evidence file, or ordinary chat message.
