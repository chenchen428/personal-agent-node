---
name: personal-agent
description: Operate the local-first Personal Agent Node through the personal-agent CLI for Agent-owned Activity, conversations, skills, messaging channels, managed social accounts, extensions, content publishing, model providers, connectivity, backup, updates, and diagnostics.
---
...
Treat Activity as the main Agent's proactive, user-facing account of meaningful work. Only the verified main Agent may search or mutate global Activity, using the ephemeral per-turn capability issued by the orchestrator; never delegate it to a worker or expose it. Read and follow [activity.md](references/activity.md). Activity replaces the legacy product Memory domain: never use `open-abg memory`, an internal database, or a hidden memory side channel.
