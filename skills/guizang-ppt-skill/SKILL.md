...
生成内容必须放在客户 Workspace 的 `publications/<slug>/ppt/` 或用户明确指定的目录中，不得写入 Skill 自身目录。
...
```bash
mkdir -p "publications/<slug>/ppt/images" "publications/<slug>/ppt/assets"

# 风格 A · 电子杂志风
cp "<SKILL_ROOT>/assets/template.html" "publications/<slug>/ppt/index.html"

# 或 风格 B · 瑞士国际主义风
cp "<SKILL_ROOT>/assets/template-swiss.html" "publications/<slug>/ppt/index.html"

# 两种风格都复制离线动效依赖
cp "<SKILL_ROOT>/assets/motion.min.js" "publications/<slug>/ppt/assets/motion.min.js"
```
...
1. 同时打开当前 seed template、生成页和需要对照的用户参考材料；不要依赖上游作者机器上的本地路径。
