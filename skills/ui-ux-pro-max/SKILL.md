...
The search script ships with the customer Workspace. Run it from the Workspace root:

```bash
python "skills/ui-ux-pro-max/scripts/search.py" "<query>" --domain <domain>
```

If `python` is not found, try `python3`, then `py -3`. The script requires Python 3.x and no external packages. If Python is unavailable, explain the requirement and continue with the documented priority rules; do not install software without the user's approval.

## Workflow

Before visual implementation, define the component boundary:

- Assign one primary responsibility to each component.
- Extract reused controls, states, cards, navigation, and interactions into shared components or hooks.
- Keep each authored component source file at 300 lines or fewer; split before delivery.
- Implement every menu destination as an independent page component and compose it from shared primitives instead of coupling multiple menu pages.
- Add architecture checks for page ownership, shared modules, and the line limit.
