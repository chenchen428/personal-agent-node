...
const examplePlan = path.join(skillRoot, "examples", "example-plan.json");
const targetRoot = path.join(repositoryRoot, "core", "app", "public", "assets", "agent-examples", "gift-advisor-report-v1");
...
export function buildGiftAdvisorTemplateExample({ check = false } = {}) {
  ...
  try {
    ...
    const generated = generateGiftAdvisorPage({ projectDir, output });
...
function compareDirectories(actualRoot, expectedRoot) {
  if (!fs.existsSync(expectedRoot)) throw new Error("built-in gift Page artifact is missing; run the build command");
  const actualFiles = listFiles(actualRoot);
  const expectedFiles = listFiles(expectedRoot);
  if (JSON.stringify(actualFiles) !== JSON.stringify(expectedFiles)) {
    throw new Error(`built-in gift Page file set drifted: expected ${expectedFiles.join(", ")}, generated ${actualFiles.join(", ")}`);
  }
  for (const name of actualFiles) {
    const actual = fs.readFileSync(path.join(actualRoot, name));
    const expected = fs.readFileSync(path.join(expectedRoot, name));
    if (!actual.equals(expected)) throw new Error(`built-in gift Page artifact drifted: ${name}`);
  }
}
