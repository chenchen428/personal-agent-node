...
- 主菜单“日程”统一呈现日程和执行记录两个视图；计划作为共享模型和日程详情保留。使用 pa-cli plan 管理单次/周期计划，calendar 查询每次发生；默认仅记录，不运行 Agent 或联系参与人。明确要求提醒或执行时设置 executionMode 和完整 executionPrompt，同一计划无需再建一份 cron。修改周期事项明确仅本次、这次及以后或整个系列，保留每次执行记录并核验持久化结果。
...
- 运行环境的执行基座、模型与服务配置由本安装的主空间统一维护；子空间只读继承，并在新回合使用最新配置。修改、检测与连接测试到主空间进行。运行服务只在内存中使用主配置凭据，不复制凭据、会话或业务数据到子空间；子空间旧运行配置保留但不生效。各空间的授权模式与 Token 额度仍独立管理。
...
- 只要创建或恢复子任务，首次确认回复必须给出工具返回的进度页面 url；进度提醒和状态答复也保留入口。远程链接不可用时准确说明原因，本机可从日程的执行记录查看，不伪造域名链接。

## 服务重启续接

服务监听成功后，只自动续接重启前start/running的主会话，保留原会话、线程和历史。沿用现有回合与子任务恢复机制，核对最近用户请求、已完成回复和工具证据；已完成则补必要回复，未完成继续，外部操作结果未知先核实。复用正在恢复的子任务，不重复下发。恢复失败或无明确终态时标记paused并记录安全原因。用户主动停止的paused不自动恢复；服务关闭造成的中断保留可恢复状态。进程内单次恢复与回合版本防止重复和新回合被旧回调覆盖，不另建队列或检查点系统。
