...
模板的专业质量底线不是“能显示平面图”或“节点数量达标”，而是完整的建筑剖切交付：连续房间地面、连续墙体围护、按真实门窗切开的洞口、阳台栏杆、密集软装和完整轴测构图必须同时成立。主视图固定采用 `professional-ssgi-ink` 渲染档位，保留环境遮蔽、建筑墨线、材质层次与完整入镜构图。建筑表现层只能从当前 Pascal 场景节点确定性派生；任何退化为孤立墙线、散落家具、平直无层次的预览或只有平面图的产物都视为质量回归。
...
- `professional-ssgi-ink` 渲染档位提供环境遮蔽、建筑墨线和空间材质对比；
- 楼层堆叠、分解和单层模式；
- 楼层选择；
- 标签显示；
- 需求和质量问题高亮；
...
- Manifest 必须声明 `professional-ssgi-ink`，Viewer 必须启用 rendered + SSGI/ink 管线，不能退回无环境遮蔽和无建筑墨线的直出模式；
