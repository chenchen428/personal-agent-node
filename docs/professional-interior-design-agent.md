...
模板的专业质量底线不是“能显示平面图”或“节点数量达标”，而是完整的建筑剖切交付：连续房间地面、连续墙体围护、按真实门窗切开的洞口、阳台栏杆、密集软装和完整轴测构图必须同时成立。主视图固定采用 `professional-mesh-ink` 渲染档位，以 WebGPU 安全的实体建筑墨线、投影、材质层次与完整入镜构图保证稳定呈现，不能依赖可能把安装态画布变空的屏幕后处理。建筑表现层只能从当前 Pascal 场景节点确定性派生；任何退化为孤立墙线、散落家具、平直无层次的预览或只有平面图的产物都视为质量回归。
...
- `professional-mesh-ink` 渲染档位提供实体建筑墨线、稳定投影和空间材质对比；
- 楼层堆叠、分解和单层模式；
- 楼层选择；
- 标签显示；
- 需求和质量问题高亮；
...
Viewer 不能保存、调用 MCP、调用 Agent、读取本地文件或访问网络。
...
- Manifest 必须声明 `professional-mesh-ink`，Viewer 必须启用 rendered shading、投影和由建筑几何派生的实体墨线；禁止使用在安装态可能退成空画布的屏幕后处理，也不能退回无材质层次和无建筑墨线的裸直出；
- 封面必须由当前模型生成轴测图，不能回退为单张平面图；
- Page 必须保留空间标签、需求/问题高亮、三维/正交视图、楼层模式、原始依据、调整标注、需求、质量和专业核验；
- Pascal Viewer 的离线包必须可直接初始化三维 Canvas，运行时异常只能触发可访问的模型派生平面降级，不能把降级图当成正常交付。
