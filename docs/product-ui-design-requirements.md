...
- 本地邮箱和 Sites：`local` 连接，安装后具备本机能力，可按需启用平台域名。
...
2. 未绑定时显示“验证公网与邮箱”。
3. 点击后打开已登录的 `personal-agent.cn` 页面进行一次授权。
...
独立自动化产品页下线，`/app/automations` 兼容跳转到连接目录。邮箱等连接可拥有系统内建轮询：本地邮箱每 60 秒扫描从上次成功游标到当前时刻的连续区间，使用短重叠窗口和 SHA-256 去重，不创建用户可编辑的自动化对象。

计划任务属于“Agent 组件 / 任务”，不增加左侧一级导航。桌面任务页标题区使用“任务列表 / 定时任务”二级视图切换；计划页只读展示明确创建的 cron/自然语言计划、时区、下次运行、工作区、通知方式、任务内容、累计次数和最近一次对应任务。每次触发仍生成普通任务，计划页不重复实现任务详情，也不提供新增、编辑、启停、删除或立即执行控件。
