...
- 本地邮箱和 Sites：`local` 连接，安装后具备本机能力；详情主操作为“配置”，点击后通过标准 DropdownMenu 选择“使用平台域名”或“使用自定义域名”，支持鼠标、触控与键盘。
...
- 连接状态按模式表达：账号连接展示授权状态，浏览器连接只展示执行器可用性，本地连接展示服务状态。
...
- 自定义域名使用三步 SOP：产品只接收一个域名并生成摘要绑定的 R2 本机配置；第一步引导用户在自管服务器准备带独立密钥的 HTTPS/WSS 反向 Relay，Node 仅主动出站连接且不需要 WireGuard、固定内网 IP 或管理员权限；第二步引导配置主域名、Space 通配子域名或 MX；第三步由 Sites 的 DNS/TLS/HTTPS 内容回读或邮箱的 MX/真实本地收件证据检测后生效。服务器、SSH、私钥、邮件内容和内部日志不得作为产品表单输入或进入验收证据。
