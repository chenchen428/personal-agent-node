...
以下工作属于重大页面变更：新增页面或主要用户流程；重大调整信息架构、导航、页面布局、响应式行为、交互状态；重构共享视觉系统；或对核心页面进行会改变用户心智模型的改造。

重大页面变更必须遵循以下顺序：

1. 在 `docs/design/<feature>/` 维护可定位版本的设计稿和实现约束；涉及交互或响应式差异时，优先交付可本地打开的 HTML 原型。
2. 覆盖受影响的桌面端和移动端体验、主要流程、空态/加载/失败/权限等重要状态，并以当前产品、CLI 和 Runtime 的真实能力为边界。
3. 由用户审核设计稿，并获得针对该版本的明确批准。沉默、局部反馈、Agent 自行判断或对旧版本的批准均不等同于批准当前设计。
4. 获得批准后才修改产品实现。实现中如需重大偏离已批准的信息架构、流程、响应式行为或视觉方向，必须先更新设计稿并重新审核。
5. 小型文案调整、保持既有体验的独立缺陷修复、无障碍修正和局部样式校正可以直接实施；一旦范围扩展为重大变更，立即返回设计审核阶段。

最终视觉和浏览器交互验收仍由用户负责。除非用户明确要求，不以自动截图、响应式视觉 QA 或自动点击代替用户审核；语义、路由、会话、安全和构建等非视觉检查仍必须执行。
...
| 连接 | `/app/connections` | P0 | 已统一 | 按分类展示连接列表、详情、Skill、CLI 与连接流程；`/app/channels` 兼容跳转 |
...
- 输入在发送动作被接受且乐观消息出现时立即清空，不等待网络请求或 Agent 回复；发送失败时恢复用户输入和待发送附件，支持重试。
...
主 Agent 对话与委派任务必须保持边界：主 Agent 对话允许继续、停止和输入新要求；桌面导航使用“核心功能”和“Agent 组件”分组，“任务”归入“Agent 组件”，任务详情只展示目标、过程、计划、结果与状态，不渲染 Composer、继续任务、停止或重试控件。

移动任务详情必须读取子任务运行时维护的独立轻量展示台账，不得在请求中扫描或分页暴露 Codex 原始对话历史。首次只取最新 20 条并在内容显示前定位到滚动容器的绝对最底部；接近顶部时用任务绑定的不透明游标继续读取更早内容，插入前页后保持当前阅读锚点。关闭状态不挂载移动菜单和遮罩，避免进入页面时出现菜单闪烁。

总览本机地址必须保持单行并可点击打开，视觉空间不足时允许省略但不得换行；数据和发布页没有内容时必须显示与场景匹配的原生 SVG、明确标题和下一步说明，首次无内容与搜索筛选无结果使用不同文案。
...
2. **长期接入：用户自管 MTA/转发规则。** 展示当前设备可复制的接收命令 `pa-cli mail ingest`、所需环境状态、允许的收件地址和测试步骤。
...
**路由：** `/app/connections?connection=:id`；`/app/channels` 仅做历史重定向。
**目标：** 用统一入口表达 Personal Agent 可以访问哪些平台，同时区分账号连接、浏览器连接和本地连接，避免把“浏览器可用”误解为“账号已授权”。

### 连接范围

- 微信：`account` 连接，可启用和扫码登录。
- 小红书：`browser` 连接，分开检测执行环境、可见登录状态和搜索/读取能力；未登录显示未连接，引导用户在原浏览器登录，不读取或保存 Cookie/Token。
- Twitter/X：`browser` 连接，复用用户现有浏览器，支持执行器检测、打开站点、搜索和线程读取，不开放发帖与互动写操作。
- Notion：`account` 连接，通过官方 CLI 完成工作区授权。
- 本地邮箱和 Sites：`local` 连接，安装后具备本机底层能力，但未配置并验证平台域名或自定义域名时统一显示黄色“未生效”且不计入“已生效”筛选；详情主操作为“配置”，点击后通过标准 DropdownMenu 选择“使用平台域名”或“使用自定义域名”，支持鼠标、触控与键盘。只有公网收件或公网访问的最终证据通过后才显示绿色。
- 后续连接：通过同一连接契约与受限 provider 进入，不为单个平台重做一级页面。

### 概览页结构

- 列表展示连接方式、状态、能力摘要和主要操作；浏览器连接区分环境待修复、平台未登录、状态待确认和已连接，只有必要登录与能力检查通过才算已生效。
- `account` 连接可以展示脱敏账号摘要；`browser` 连接仅展示登录是否确认，不展示账号标识、登录时间、页面正文或浏览器凭据。
- 计划中连接放独立“可用扩展”区域，不与故障连接混合。

### 详情/连接流程

- 基本信息：provider、版本、`accessMode`、只读/可写、来源（内置/插件）。
- 连接状态按模式表达：账号连接展示授权状态，浏览器连接分别展示环境、平台登录与只读能力；本地邮箱和 Sites 以域名及最终公网能力是否验证通过作为生效口径，本机进程存在不等于连接已生效。
- 能力：对话、页面、图片、文件、搜索、读取等。
- 权限：读取、外部写入、通知范围。
- 操作由连接模式决定：账号连接可连接或重新授权；浏览器连接只提供检测、打开、搜索和读取；本地连接提供状态与受控配置。
- 小红书和 X 通过固定官方站点引导用户人工登录，允许有限的只读状态复查；不在产品中代填或保存账号凭据，不提供浏览器账号清空。平台网页自己的扫码与验证由用户完成。
- 浏览器读取 CLI 随 Personal Agent 安装和升级，界面不得要求用户安装 Node.js、npm 或全局 CLI；首次使用仅在 Chrome 中确认一次浏览器读取权限，并在完成后重新检测。
- 自定义域名使用三步 SOP：第一步明确说明用户需要另备有公网 IP、持续在线的 Linux 服务器并安装 Relay、Nginx 与 HTTPS；客户端展示并可复制当前版本 GitHub Release 中 `personal-agent-relay-install.sh` 的固定下载与执行命令，该资产内嵌同版本 Relay 并进入 SHA256SUMS、Sigstore 与 provenance，禁止引用可变分支。Relay 首次安装生成并只展示一次连接密钥，用户将它粘贴到客户端密码框，客户端保存明文而服务器只持久化摘要。第二步引导配置主域名、Space 通配子域名或 MX；第三步由 Sites 的 DNS/TLS/HTTPS 内容回读或邮箱的 MX/真实本地收件证据检测后生效。R2 本机配置绑定域名与密钥摘要，Node 仅主动出站连接且不需要 WireGuard、固定内网 IP 或管理员权限；服务器地址、SSH、私钥、邮件内容和内部日志不得作为产品表单输入或进入验收证据。
- 平台域名与自定义域名互斥。选择自定义域名会停止自动平台绑定重试并切换为 `self-hosted-edge`；移除后保留本地 Site、邮件归档和 Workspace 数据。
...
2. 未绑定时由桌面端在后台发起一次 Cloud 授权和资源分配，不要求用户点击开始。
3. 已登录的 `personal-agent.cn` 会校验一次性请求并自动回调本机；需要登录、MFA、风控或确认时，静默流程立即失败。
4. Node 拉取公开资源状态，并自动启动 Sites 与邮箱域名绑定验证。
5. 同时展示公网域名和 Agent 邮箱结果；部分成功时明确哪一项未就绪。
6. 只有静默授权或域名验证失败后才显示手动“重新连接”；Cloud 不可达时保留本机能力，不能形成重试死循环。
...
**目标：** 用户知道当前版本、是否有更新、更新会改变什么，并能查看最近安装结果。

页面包含当前 Core 版本、可用版本、Workspace 兼容性、变更摘要、更新前检查、下载/安装进度和验收结果。客户端不提供人工恢复旧版本入口；候选版本启动失败时由运行时自动安全回退。更新是 R2，任何更新都不能删除 Workspace 数据。

发布记录按时间列出 release ID、版本、验收状态、时间、变更和证据。桌面使用列表 + 详情，移动使用列表到详情导航。两个路由最终纳入统一 Shell，不保留旧版孤立导航。

## 5.11 连接内建轮询与计划任务

**路由：** `/app/connections`、`/app/workers`、`/app/workers/schedules`；`/app/schedules` 仅保留兼容跳转。
**优先级：** 连接已统一；桌面计划任务查看入口按 Desktop V7.8 纳入任务模块。

独立自动化产品页、通用规则引擎和用户可编辑自动化对象全部下线，`/app/automations` 兼容跳转到 `/app/workers/schedules`。邮箱等连接可拥有系统内建轮询：本地邮箱每 60 秒扫描从上次成功游标到当前时刻的连续区间，使用短重叠窗口和 SHA-256 去重；通过安全检查后直接创建普通任务，不经过独立自动化规则、运行或模板模型。

“自动化”属于“Agent 组件 / 任务”，不增加左侧一级导航。它是定时任务能力的用户可见名称：桌面任务页标题区使用“任务列表 / 自动化”二级视图切换；自动化页只读展示明确创建的 cron/自然语言计划、时区、下次运行、工作区、通知方式、任务内容、累计次数和最近一次对应任务。每次触发仍生成普通任务，自动化页不重复实现任务详情，也不提供新增、编辑、启停、删除或立即执行控件；CLI 和持久化合同继续使用 `cron` / scheduled task。

计划任务在移动端使用列表到详情导航。时间必须同时显示用户时区和绝对时间 tooltip。
...
```text
主 Agent 接收目标 -> 委派任务 -> Agent 运行 -> 实时消息/工具记录 -> 完成 -> 可回看
```
