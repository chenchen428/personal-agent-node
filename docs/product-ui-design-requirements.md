...
- 小红书：`browser` 连接，复用用户现有浏览器，支持执行器检测、打开站点、搜索和笔记读取，不检查或保存账号登录态。
...
- 列表展示连接方式、状态、能力摘要和主要操作；浏览器连接使用“浏览器可用 / 需要浏览器确认 / 浏览器不可用”。
- `account` 连接可以展示脱敏账号摘要；`browser` 连接不得展示账号、登录时间或“已登录”。
- 计划中连接放独立“可用扩展”区域，不与故障连接混合。

### 详情/连接流程

- 基本信息：provider、版本、`accessMode`、只读/可写、来源（内置/插件）。
- 连接状态按模式表达：账号连接展示授权状态，浏览器连接只展示执行器可用性；本地邮箱和 Sites 以域名及最终公网能力是否验证通过作为生效口径，本机进程存在不等于连接已生效。
- 能力：对话、页面、图片、文件、搜索、读取等。
- 权限：读取、外部写入、通知范围。
- 操作由连接模式决定：账号连接可连接或重新授权；浏览器连接只提供检测、打开、搜索和读取；本地连接提供状态与受控配置。
- 扫码仅用于明确支持扫码授权的账号连接。小红书和 Twitter/X 浏览器连接不显示扫码、登录轮询、断开或账号绑定操作。
