...
- 本机安装与 Core 链接。
- 远程访问密码（仅在启用公网或隧道访问时设置；本机桌面客户端免密）。
- Codex 安装、版本、登录和真实对话。
...
首次检测、检测中、本机免密访问、远程密码错误/成功、Codex 缺失/未登录/真实回复、Cloud 未绑定/授权中/成功/服务不可达/部分成功、邮件未接入、全部完成、手机表单。
...
**路由：** `/login`  
**目标：** 远程或隧道访问时用远程访问密码验证身份并安全返回原页面；桌面客户端直连 localhost 不进入登录页。
