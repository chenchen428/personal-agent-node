...
进入桌面工作区前，一次性预取菜单首屏的只读数据（4个并发、总计最多8秒），失败的接口保留错误供页面重试，不伪造成功。查询数据、默认任务详情等依赖请求也在同一启动阶段准备。之后点击菜单直接读取空间内快照，可见页后台更新，不出现逐页打开动画。首次启动和真正切换空间仍需要一次初始化；业务操作、查询新条件及附件打开可有局部反馈。
