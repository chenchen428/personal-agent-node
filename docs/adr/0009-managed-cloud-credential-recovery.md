...
| `refreshing` | Expiry window or Broker 401 | Single-flight refresh; no browser | `healthy`/`recovered`, `degraded`, or `reauth_required` |
...
| `reauth_required` | Missing, expired, revoked, replayed, or binding-invalid refresh | Stop the retry storm and expose the governed managed-authorize action | `authorizing` |
| `authorizing` | Locally approved R2 `cloud connect --repair` | Open the official same-origin device authorization page and poll | `recovered` or `failed` |
...
- **Device substitution:** refresh checks Site, tenant account through the Site, installation id, Space id, and the Site's enrolled device id before rotation.
