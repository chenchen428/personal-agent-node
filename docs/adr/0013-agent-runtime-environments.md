...
运行设置的“运行环境”统一承载基座、模型、推理强度和自定义服务配置。通用设置移除独立的“Agent 执行”模型配置。主工作区统一选择 Codex 或 Claude Code 并维护双基座配置；子工作区只读应用同一配置，不再保存独立生效的模型或服务设置。此次交互与视觉由用户在研发、测试和本机重装完成后验收。
...
- 非敏感配置位于注册主 Space 的 `config/runtime-environments.json`，含 schema、revision、engine 和双基座 profile。首次读取兼容主工作区已有 Codex 模型与推理设置。子空间旧配置保留以便回滚，但不再参与有效配置选择。
- 授权凭据位于注册主 Space 的 `secrets/runtime-environments/`，配置仅保存随机凭据引用；HTTP 只返回 `credentialConfigured`。空白输入保留已有凭据，显式清除才移除。变更服务来源不能把旧凭据自动发送到新来源。
...
运行设置仅注册的 personal 主工作区可保存、检测和测试。子工作区接口返回 readOnly、inherited 和配置来源，只读显示主配置；请求修改在解析草稿或接触凭据之前拒绝。父配置更新后子空间下一回合重新读取，正在执行的回合保留启动快照。凭据由运行服务引用到对应执行环境，不复制进子空间配置或对外响应。业务数据、长期记忆、会话历史、授权模式和Token额度继续按Space独立。
