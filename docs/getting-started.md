...
Set `TAG=v0.2.0-beta.16` and open the matching [GitHub Release](https://github.com/chenchen428/personal-agent-node/releases/tag/v0.2.0-beta.16). A customer machine does not need Node.js, npm, Git, a source checkout, or a development Agent.
...
| Linux x86-64 | `personal-agent-node-v0.2.0-beta.16-linux-x64.tar.zst` |
| Linux ARM64 | `personal-agent-node-v0.2.0-beta.16-linux-arm64.tar.zst` |
...
Each package contains the Go setup executable, stable recovery CLI, the exact Node.js `22.23.1` runtime, the platform Tauri 2 application, and the immutable application payload. Setup verifies embedded checksums, removes any legacy managed service, stages the release, initializes the Workspace, switches `current` while retaining `previous`, and commits the stable application launcher, installation state, product icon, and desktop entries. It does not register a separate per-user service. When the desktop application starts, it launches and monitors the bundled Node supervisor without opening a terminal; when the application exits, it stops that supervisor and its child processes. Program releases and user-owned Workspace data remain separate storage boundaries for safe upgrades and uninstall, not separate products.
