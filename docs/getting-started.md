...
Personal Agent Node must start in local-only mode without contacting `personal-agent.cn`. Managed Cloud enrollment is an optional provider selected explicitly by the user. Fresh release installs and upgrades provision a missing local mail-ingress token, render current-following CLI shims, and non-destructively copy legacy `mail-ingress/` data into `mail/` while retaining the old source for rollback.
