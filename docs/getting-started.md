...
On Windows, run the installer and follow the native wizard; it shows verification and installation progress, creates Start menu and desktop icons, and offers to launch Personal Agent on the final page. On macOS, open the package. On Linux, unpack the matching archive with the desktop archive manager or `tar --zstd`, then run `./personal-agent-setup` from the extracted directory.

Each package contains the Go setup executable, stable CLI and desktop launchers, the exact Node.js `22.23.1` runtime, the platform Tauri 2 shell, and the immutable application payload. Setup verifies embedded checksums, stops any previous managed or development supervisor, stages the release, initializes the Workspace, switches `current` while retaining `previous`, registers the per-user service, and waits for the local gateway. Only after those checks pass does it atomically commit the stable launchers, installation state, product icon, and desktop entries. A failed candidate restores the earlier integration and service. The shell uses the system WebView and does not own the background service. The only default home is `~/.personal-agent`: product releases live under `core/`, while Harness, plugins, files, databases, mail and other user-owned state live under `workspace/`.
