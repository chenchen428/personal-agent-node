...
The ordinary response to a policy-authorized inbound Personal WeChat conversation is a narrow current-session reply path. The orchestrator may call `sendText`, `sendImage`, and `sendFile` directly on this connector only for the already bound conversation recipient after validating a canonical main-Agent final-reply envelope and its managed objects. This does not expose a general write API and does not authorize another recipient. Manual CLI sends, proactive messages, account changes, and cross-conversation writes continue to require their R2/R3 operation plan and local human approval.
