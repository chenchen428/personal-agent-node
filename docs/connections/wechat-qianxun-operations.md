...
先按[千寻 Pro 官方快速入门](https://daenmax.github.io/qxpro-doc/doc/start/)安装千寻 Pro 与受支持的 PC 微信版本，申请试用或购买授权，启动千寻 Pro、完成微信登录并配置 HTTP API。在“连接 → 个人微信”复制当前实例的消息回调地址，填入千寻 Pro 的 HTTP 事件回调配置；不要根据浏览器开发端口猜测回调端口。
