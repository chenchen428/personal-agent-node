...
export function installPersonalAgentCommand({ installRoot, dataRoot, platform = process.platform, env = process.env, homeDir = os.homedir(), fileSystem = fs } = {}) {
  ...
  const runtimeDataRoot = targetPath.resolve(dataRoot || env.PRIVATE_SITE_DATA_ROOT || targetPath.join(homeDir, '.personal-agent'));
  ...
  const content = platform === 'win32'
    ...
    : `#!/bin/sh\nPRIVATE_SITE_INSTALL_ROOT='${root.replaceAll("'", `'"'"'`)}' PRIVATE_SITE_DATA_ROOT='${runtimeDataRoot.replaceAll("'", `'"'"'`)}' exec node '${entrypoint.replaceAll("'", `'"'"'`)}' "$@"\n`;
  ...
  return { commandPath, binDir, entrypoint, dataRoot: runtimeDataRoot, followsCurrent: true };
}
