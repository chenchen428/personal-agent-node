...
for (const file of ['AGENTS.md', 'README.md', 'DESIGN.md', 'docs/adr/0001-node-product-boundary-freeze.md', 'docs/adr/0003-core-workspace-next-architecture.md', 'agents', 'registry/agents.json', 'registry/projects.json', 'registry/delivery.json', 'registry/skills.json', 'registry/behavior-baselines.json', 'registry/capabilities.json', 'registry/routes.json', 'registry/extensions.json', 'registry/commands.json', 'schemas/personal-agent/agents.schema.json', 'schemas/personal-agent/agent-profile.schema.json', 'workspace/AGENTS.md', 'scripts/agent-guard.mjs', 'scripts/project-guard.mjs', 'scripts/architecture-guard.mjs', 'scripts/skill-guard.mjs', 'scripts/skill-tree.mjs', 'scripts/verify-behavior-baselines.mjs', 'scripts/setup-agent-bridge.sh']) checks.push({ name: `harness file ${file}`, ok: exists(file) });
...
try { execFileSync(process.execPath, ['scripts/agent-guard.mjs'], { cwd: root, stdio: 'pipe' }); checks.push({ name: 'specialist Agent registry', ok: true }); } catch (error) { checks.push({ name: 'specialist Agent registry', ok: false, detail: error.stderr?.toString().trim() || error.stdout?.toString().trim() }); }
