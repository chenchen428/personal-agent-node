...
PRIVATE_SITE_EDGE_CONFIG_DIR="$EDGE_CONFIG_DIR" PRIVATE_SITE_EDGE_STATE_DIR="$EDGE_STATE_DIR" \
  node "$RELEASE_DIR/core/edge/bin/private-site-edge.mjs" render >/dev/null
nginx -t
systemctl reload nginx
PERSONAL_AGENT_CURRENT_ROOT="$RELEASE_DIR" bash "$RELEASE_DIR/core/edge/scripts/install-renewal-cron.sh"
