...
Production runs only from the bundled entrypoints inside `~/.private-site-node/current`. The `private-site-node` supervisor starts the Bridge and Worker as background children of the user-owned platform service. ECS never runs this project.
