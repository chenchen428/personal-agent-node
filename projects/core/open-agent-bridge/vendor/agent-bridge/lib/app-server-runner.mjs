...
import { createAppServerMapperState, mapMessage, threadIdFromResult, turnIdFromResult, completionPayload, collabReceiverThreadIds } from './app-server-mapper.mjs';
 ...
 */
export async function steerActiveTurn(sessionId, content, onSessionEvent) {
...
function appServerClientOptions(config) {
  return {
    ...
    args: Array.isArray(config.appServerArgs) ? config.appServerArgs : undefined,
    cwd: config.workspace,
    env: config.agentEnv,
    transport: deriveAppServerTransport(config),
    socketPath: config.appServerSocketPath,
    log: config.appServerLog,
  };
}
...
async function ensureThread(session, config) {
  ...
  if (session.threadId) { // known thread but not live in the current child -> resume from disk rollout
    const res = await client.request('thread/resume', { threadId: session.threadId, cwd: config.workspace });
  ...
  if (known) {
    try {
      const res = await client.request('thread/resume', { threadId: known, cwd: config.workspace });
...
function threadStartParams(config) {
  const params = {
    cwd: config.workspace,
...
async function listSkills(config) {
  ...
  const res = await getAppServerClient(appServerClientOptions(config || {})).call('skills/list', {
