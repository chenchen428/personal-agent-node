...
test("Node distribution verification uses bundle-stable channel markers", () => {
  const verifier = fs.readFileSync(fileURLToPath(new URL("../../../../scripts/verify-private-site-node-dist.mjs", import.meta.url)), "utf8");
  assert.match(verifier, /server\.includes\("data-status"\)/);
  assert.match(verifier, /server\.includes\("data-detail"\)/);
  assert.doesNotMatch(verifier, /serverBundle\.includes\("在微信中发起登录协作"\)/);
});
