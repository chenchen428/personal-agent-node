...
test("local cleanup delegates to catalog GC instead of deleting by mtime", () => {
  const service = fs.readFileSync(path.join(workspaceRoot, "projects", "core", "open-agent-bridge", "src", "managed-files", "service.js"), "utf8");
  const server = fs.readFileSync(path.join(workspaceRoot, "projects", "core", "open-agent-bridge", "src", "server", "server.ts"), "utf8");
