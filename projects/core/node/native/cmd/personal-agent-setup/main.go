...
func installCommand(args []string) {
	...
	installRoot := set.String("install-root", filepath.Join(home, ".private-site-node"), "immutable installation root")
	dataRoot := set.String("data-root", filepath.Join(home, ".personal-agent"), "mutable data root")
