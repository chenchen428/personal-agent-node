...
func main() {
	...
	if dataRoot == "" {
		...
		dataRoot = filepath.Join(home, ".personal-agent")
	}
	...
	entrypoint := filepath.Join(current, "projects", "core", "node", "bin", "personal-agent.mjs")
	command := exec.Command(node, append([]string{entrypoint}, os.Args[1:]...)...)
	command.Env = append(os.Environ(), "PRIVATE_SITE_INSTALL_ROOT="+installRoot, "PRIVATE_SITE_DATA_ROOT="+dataRoot)
