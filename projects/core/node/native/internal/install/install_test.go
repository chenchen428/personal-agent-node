...
func TestVerifyReleaseRejectsChangedFiles(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(release, "projects/core/node/bin/personal-agent.mjs"), []byte("changed"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func fixtureRelease(t *testing.T, releaseID string) string {
	...
	files := map[string][]byte{
		"projects/core/node/bin/personal-agent.mjs": []byte("// personal-agent"),
		"projects/core/node/bin/private-site.mjs":   []byte("// private-site"),
		"SBOM.cdx.json": []byte(`{"bomFormat":"CycloneDX"}`),
	}
	manifest, _ := json.Marshal(map[string]any{"schemaVersion": 1, "releaseType": "private-site-node", "releaseId": releaseID, "revision": strings.Repeat("a", 40), "dirty": false})
