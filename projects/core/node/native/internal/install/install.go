...
func Install(ctx context.Context, opts Options, runner Runner) (result Result, returnedErr error) {
	...
	defer func() {
		...
		if serviceAttempted {
			...
			if oldCurrent != "" {
				...
				oldPrivateSite := filepath.Join(oldCurrent, "projects", "core", "node", "bin", "private-site.mjs")
	...
	privateSite := filepath.Join(target, "projects", "core", "node", "bin", "private-site.mjs")
	env := envFor(resolved)
...
func Rollback(ctx context.Context, installRoot, platform string, runner Runner) (Result, error) {
	...
	if state.Service != "" && state.Service != "skipped" && state.DataRoot != "" {
		...
		privateSite := filepath.Join(previousTarget, "projects", "core", "node", "bin", "private-site.mjs")
...
func VerifyRelease(root string) (Manifest, error) {
	...
	if manifest.SchemaVersion != 1 || manifest.ReleaseType != "private-site-node" || manifest.ReleaseID == "" || manifest.Dirty {
	...
	for _, required := range []string{"release-manifest.json", "projects/core/node/bin/personal-agent.mjs", "projects/core/node/bin/private-site.mjs", "SBOM.cdx.json"} {
...
func envFor(opts Options) []string {
	return append(os.Environ(), "PRIVATE_SITE_INSTALL_ROOT="+opts.InstallRoot, "PRIVATE_SITE_DATA_ROOT="+opts.DataRoot, "PRIVATE_SITE_CLI_BIN="+filepath.Join(opts.InstallRoot, "bin"))
}
