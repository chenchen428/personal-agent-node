...
import { renderShim } from "../src/cli-shims.mjs";
...
test("mail status is redacted, local-only, and bounds archive accounting", () => {
  ...
  const entrypoint = path.join(installRoot, "current", "projects", "core", "open-agent-bridge", "bin", "oab-mail-ingest.mjs");
  ...
  try {
    ...
    const canonicalEntrypoint = path.join(fs.realpathSync(installRoot), "current", "projects", "core", "open-agent-bridge", "bin", "oab-mail-ingest.mjs");
