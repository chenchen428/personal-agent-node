...
test("workspace .local physically owns all Site files", () => {
  ...
  try {
    ...
    assert.equal(fs.lstatSync(path.join(config.dataRoot, "files")).isSymbolicLink(), true);
    assert.equal(fs.readFileSync(path.join(config.dataRoot, "files", "managed", "asset.txt"), "utf8"), "managed");
    assert.equal(fs.readFileSync(path.join(config.agentWorkspaceRoot, ".local", "files", "inbound", "message.txt"), "utf8"), "inbound");
    assert.equal(fs.realpathSync(path.join(config.dataRoot, "files")), fs.realpathSync(path.join(config.agentWorkspaceRoot, ".local", "files")));
  } finally {
...
test("workspace file migration refuses conflicting content", () => {
  ...
  try {
    const source = path.join(config.dataRoot, "files", "managed", "same.txt");
    const target = path.join(config.agentWorkspaceRoot, ".local", "files", "managed", "same.txt");
