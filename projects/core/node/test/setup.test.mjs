...
import { inspectRemoteConnectivity, setupDiagnostics, setupStatus, writeWebConversationAcceptance } from '../src/setup.mjs';
...
test('setup status separates console, Agent, remote, mail, and optional WeChat readiness', async () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(dataRoot, 'runtime', 'supervisor.json'), `${JSON.stringify({
      pid: 123,
      status: 'running',
      components: Object.fromEntries(['personal-agent-control', 'open-agent-bridge', 'open-agent-bridge-worker', 'workspace-admin-panel', 'private-site-gateway'].map((name, index) => [name, { pid: index + 10 }])),
    })}\n`);
